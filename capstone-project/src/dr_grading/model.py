...
What it does, start to finish:
  ...
  1. Load the preprocessed arrays from ``data/preprocessed`` (dr_grading/data.py).
  ...
  5. Evaluate on the held-out test split with QWK, per-class sensitivity /
     specificity and ROC-AUC (dr_grading/evaluate.py).
...
from dr_grading.artifacts import log_run_artifact_location, resolve_artifact_paths
from dr_grading.data import PreprocessedDataset, load_preprocessed_data
from dr_grading.evaluate import (
    evaluate_predictions,
    format_report,
    metrics_for_logging,
    quadratic_weighted_kappa,
)
from dr_grading.tracking import (
    configure_mlflow,
    dataset_summary_params,
    flatten_params,
    load_training_config,
)
from dr_grading.paths import ARTIFACTS_DIR, DEFAULT_TRAINING_CONFIG, PROJECT_DIR
...
def run_training(config: dict[str, Any]) -> dict[str, Any]:
    ...
    artifact_paths = resolve_artifact_paths(config, artifacts_dir=ARTIFACTS_DIR)
    ...
    with run_ctx:
        if mlflow is not None:
            ...
            log_run_artifact_location(mlflow, artifact_paths.run_dir)
