...
\begin{table}[t]
  \centering
  \caption{Per-class image counts at each stage of the pipeline. Grades follow
  the five-point clinical DR scale (0--4). \textbf{These counts are stale:}
  they were produced by an earlier version of the pipeline that applied quality
  filtering, and are retained only for reference. They must be regenerated from
  a run of the current procedure, which discards no images (so the ``raw'' row
  is the input to SMOTE) and balances every training grade to the majority
  count of \num{1434}, giving \num{7170} training images in total.}
  \label{tab:counts}
